...
const WriteModule = () => {
  ...
  const round = useQuestionRound({ module: "write", subject, warriorSeconds: 60 });
  ...
  return (
    <FramerMotion.motion.div className={styles.page} initial={{ opacity: 0, x: -80 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 80 }} transition={{ duration: 0.4 }}>
        ...
        {mode === "warrior" && !answered && (
          <div style={{ margin: "8px 0 12px", display: "flex", alignItems: "center", gap: 10, width: "100%", maxWidth: 700 }}>
            ...
            <div style={{ flex: 1, height: 8, background: "rgba(255,255,255,0.15)", borderRadius: 99, overflow: "hidden" }}>
              <div style={{ height: "100%", width: `${(timeLeft / warriorSeconds) * 100}%`, background: timeLeft <= 15 ? "#e74c3c" : "#6c63ff", borderRadius: 99, transition: "width 1s linear" }} />
            </div>
          </div>
        )}

        <AnimatePresence mode="wait">
          <FramerMotion.motion.div key={current.id} initial={{ opacity: 0, y: 40 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -40 }} transition={{ duration: 0.4 }} style={{ width: "100%", display: "flex", justifyContent: "center" }}>
      ...
      <AnimatePresence>
        {roundDone && (
          <RoundComplete
            module="write" subject={subject || "general"} mode={mode}
