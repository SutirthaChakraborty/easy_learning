...
const getAllQuestions = async (req, res) => {
  try {
    ...
    const { filter: visibility, noOrgQuestions } = await buildQuestionVisibilityFilter(req.user.email, 'listen', 'science')
...
const getQuestionById = async (req, res) => {
  try {
    const lang = req.query.lang || 'en'
    const { filter: visibility } = await buildQuestionVisibilityFilter(req.user.email, 'listen', 'science')
    const question = await ListenScience.findOne({ id: Number(req.params.id), status: 'approved', ...visibility })
