...
// GET /api/game/memory-match/cards?count=8
  ...
  try {
    ...
    const deck = shuffle(
      picked.flatMap(item => [
        ...
        { cardId: `${item.id}-label`,  pairId: item.id, type: 'label', content: item.label }
      ])
    )
...
const checkMatch = async (req, res) => {
  try {
    ...
    if (isMatch) {
      ...
      return res.json({ success: true, match: true, emoji: item.emoji, label: item.label })
    }
