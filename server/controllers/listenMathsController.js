...
const getAllQuestions = async (req, res) => {
  try {
    ...
    const { filter: visibility, noOrgQuestions } = await buildQuestionVisibilityFilter(req.user.email, 'listen', 'maths')
...
const getQuestionById = async (req, res) => {
  try {
    const lang = req.query.lang || 'en'
    const { filter: visibility } = await buildQuestionVisibilityFilter(req.user.email, 'listen', 'maths')
    const question = await ListenMaths.findOne({ id: Number(req.params.id), status: 'approved', ...visibility })
