...
const getAllItems = async (req, res) => {
  try {
    const { filter: visibility, noOrgQuestions } = await buildQuestionVisibilityFilter(req.user.email, 'write', 'english')
    ...
    res.json({ success: true, count: items.length, data: items, noOrgQuestions })
  } catch (err) {
...
const getItemById = async (req, res) => {
  try {
    const { filter: visibility } = await buildQuestionVisibilityFilter(req.user.email, 'write', 'english')
